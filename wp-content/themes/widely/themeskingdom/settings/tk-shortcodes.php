	...
	*/
	...
	function showPirobox($atts, $content = null){
		...
		if(strpos($link,'youtube') > 0){
			...
			$link = 'http://www.youtube-nocookie.com/v/'.$newlink[0];
			$video = 1;
		}
	...
	function showList($atts, $content = null){
		...
		if($type == "ol"){
			return '<ol class="ullist">'.$content.'</ol>';
		}else{
	...
	function showQuote($atts, $content = null){
		return '<div class="quote"><div class="quote-image"></div><div class="quote-text">'.do_shortcode($content).'</div></div>';
	}
	...
	function cellimage($atts, $content = null ) {
		...
		<a title="'.$title.'" rel="single" class="pirobox first last" href="'.do_shortcode($content).'"><img title="'.$title.'" alt="_" src="'.$template_directory.'/script/timthumb.php?src='.do_shortcode($content).'&w=35&h=35&zc=0&q=100"></a>
		</div>';
	...
	function cellimagesmall($atts, $content = null ) {
	    ...
	    return '<div class="cell_image_front">
		<a title="'.$title.'" rel="single" class="pirobox first last" href="'.do_shortcode($content).'"><img title="'.$title.'" alt="_" src="'.$template_directory.'/script/timthumb.php?src='.do_shortcode($content).'&w=27&h=27&zc=0&q=100"></a>
		</div>';
	...
	function showimg($atts, $content= null ) {
		...
		return '<a title="'.$title.'" rel="single" class="pirobox first last" href="'.do_shortcode($src).'" rel="iframe-640-505" rev="0"><span class="shortcodeimg-holder" style="float:'.$align.';margin:'.$margin.';padding:'.$padding.';border-width:'.$border.';"><img title="'.$title.'" alt="_" src="'.$template_directory.'/script/timthumb.php?src='.do_shortcode($src).'&amp;w='.$width.'&amp;h='.$height.'&amp;zc=1&amp;q=100"></span></a>';
	...
	function contactform($atts, $content=null){
		...
		<div class="contact-form-left" style="margin:0px 0px 0px 0px;">
					<form action="#" method="post" id="commentform">
						...
						<p>
							...
							<input type="text" spellcheck="false" name="email" id="email" value="E-mail" class="contact_input_text last" tabindex="2" />
						...
						<p>
							...
							<textarea name="comment" spellcheck="false" id="message" class="input form-textarea" tabindex="3">Message</textarea>
					...
					<div class="contact-send-button"><div class="contact-send-button-left"></div><div class="contact-send-button-center">
						<input type="submit" class="contact-submit" name="submit-comment"  value="Send" />
					</div><div class="contact-send-button-right"></div></div>	
...
jQuery(document).ready(function() {
	...
	jQuery("form").submit(function(e) {
 		...
 		if(contactname == "Name (required)" || contactname == "Name"){
 			error = 1;
 			msg = "<span class=\"status-box\"><span class=\"error\"><div class=\"error-icon ajaxicon\">Error</div><p class=\"warning-title\">'.$insertname.'</p></span></span>";
 		}
 		var reg = /^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/;
 		 ...
 		 if(reg.test(email) == false) {
 			...
 			msg = msg + "<span class=\"status-box\"><span class=\"error\"><div class=\"error-icon ajaxicon\">Error</div><p class=\"warning-title\">'.$validemail.'</p></span></span>";
 		}
 		...
 		if(message == "Message"){
 			...
 			msg = msg + "<span class=\"status-box\"><span class=\"error\"><div class=\"error-icon ajaxicon\">Error</div><p class=\"warning-title\">'.$insertmessage.'</p></span></span><br/>";
 		}
 		e.preventDefault();
		...
		$form .= '	jQuery(\'.ajaxmessage\').empty().append(\'<span class=\"status-box\"><span class=\"success\"><div class=\"success-icon ajaxicon\">Success</div><p class=\"warning-title\">'.$messagesent.'</p></span></span><br/>\');';
...
$panes[] = '<div class="pane" id="tab-shortcode-'.$i.'"><br />'.$tab['content'].'</div>';
$i++;
}
