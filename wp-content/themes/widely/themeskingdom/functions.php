		...
		<div class="single-comment-holder">
			<div class="comment-left-side">
				<div class="single-comment-content">
					<div class="single-comment-title"><span class="bold">BY:</span> <a href="<?php echo $author_email?>"><?php printf(__('%s'), comment_author()) ?></div></a>
						<div class="single-comment-date"><span class="bold">ON:</span> <a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ) ?>">
						<?php the_time('M j Y') ?>
					</a></div>
<!--				<div class="single-comment-post">@<a href="<?php echo get_site_url().'/category/'.$cat_slug; ?>"><?php echo $post_category; ?></a></div>-->
				</div>
			<div class="reply"><?php comment_reply_link(array_merge( $args, array('depth' => $depth, 'max_depth' => $args['max_depth']))) ?></div>  
			<span class="commentmetadata-edit" style="float:left;"><?php edit_comment_link(__('Edit'),'  ','') ?></span>
			</div>
	...
	function comments_popup( $zero = false, $one = false, $more = false, $css_class = '', $none = false ) {
		...
		echo "<div class='comment_popup_wrap'><span class='comment_popup'>";
   ...
   function tk_pagination($first=1,$last=1,$middle=5,$baseURL=false,$wp_query=false ) {
      ...
      if ( $wp_query->found_posts > $wp_query->query_vars["posts_per_page"] ) {// Only necessary if there's more posts than posts-per-page
              ...
              if ( $page > 1 ) { // Previous link?
                      echo '<li class="previous portfolio-pageing-prev"><a href="'.$baseURL.(($page>=2)?('page/'.($page-1).'/'):'').$qs.'"></a></li>';
              }
              $dots=false;
              for ( $i=1; $i <= $wp_query->max_num_pages; $i++ ){ // Loop through pages
                      if($i<=$first || $i<=$middle && $page<$middle || $i>$wp_query->max_num_pages-$last || $i>$wp_query->max_num_pages-$middle && $page>$wp_query->max_num_pages-$middle+1 || $i>$page-ceil($middle/2) && $i<=$page+floor($middle/2)){
                              if ( $i == $page ) { // Current page or linked page?
                                      echo '<li class="active-portfolio-pageing portfolio-pageing">'.$i.'</li>';
                              } else {
                                      echo '<li class="listnum portfolio-pageing"><a href="'.$baseURL.(($i!=1)?('page/'.$i.'/'):'').$qs.'">'.$i.'</a></li>';
                              }
                              $dots=false;
                      }elseif(!$dots){
              ...
              if ( $page < $wp_query->max_num_pages ) { // Next link?
                      echo '<li class="next portfolio-pageing-next"><a href="'.$baseURL.'page/'.($page+1).'/'.$qs.'"></a></li>';
              }
